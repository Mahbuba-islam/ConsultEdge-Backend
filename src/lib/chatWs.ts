...
const resolveSocketIdentity = async (req: IncomingMessage) => {
...
export class ChatWsHub {
  ...
  private register() {
    this.wss.on("connection", async (socket: WsClient, req) => {
      ...
      const userSockets = this.socketsByUser.get(userId) ?? new Set<WsClient>();
  ...
  emitToRoom(roomId: string, eventName: string, payload: unknown) {
