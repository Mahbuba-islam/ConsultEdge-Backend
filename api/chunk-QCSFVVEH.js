...
var getAllExpertSchedules = async (query, user) => {
  const baseQuery = user?.role === Role.ADMIN ? { isDeleted: false, ...query } : {
    ...query,
    isDeleted: false,
    isBooked: false,
    isPublished: true
  };
  const qb = new QueryBuilder(prisma.expertSchedule, baseQuery, {
    filterableFields: expertScheduleFilterableFields,
    searchableFields: expertScheduleSearchableFields
  });
  return await qb.search().filter().paginate().dynamicInclude(expertScheduleIncludeConfig).sort().fields().excute();
...
var getAllExpertSchedules2 = catchAsync(async (req, res) => {
  const result = await expertScheduleService.getAllExpertSchedules(
    req.query,
    req.user
  );
  sendResponse(res, {
