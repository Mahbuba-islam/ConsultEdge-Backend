...
// src/modules/ai/prompts/searchPrompt.ts
var searchPrompt = (input) => `
...
var heuristicSearch = (input) => {
  const q = input.query.toLowerCase();
  const tokens = q.split(/\s+/).filter(Boolean);
  const experts = input.experts.map((e) => {
    const haystack = [
      e.name,
      e.industry ?? "",
      (e.expertise ?? []).join(" "),
  ...
  const industries = (input.industries ?? []).map((name) => ({
    name,
...
var hydrateSearchInput = async (input) => {
  const providedExperts = (input.experts ?? []).slice(0, 200);
  if (providedExperts.length > 0) {
    const industries2 = input.industries && input.industries.length > 0 ? input.industries : Array.from(
      new Set(
        providedExperts.map((expert) => expert.industry).filter((industry) => !!industry)
      )
    ).slice(0, 100);
    return {
      query: input.query,
  ...
  const experts = dbExperts.map((expert) => ({
    id: expert.id,
    name: expert.fullName,
    industry: expert.industry.name,
    expertise: expert.title ? [expert.title] : [],
    bio: expert.bio ?? void 0
  }));
  const industries = input.industries && input.industries.length > 0 ? input.industries : Array.from(new Set(experts.map((expert) => expert.industry).filter(Boolean))).slice(0, 100);
  return {
    query: input.query,
    experts,
    industries
...
var search = async (input) => {
  ...
  try {
    ...
    if (!data || !Array.isArray(data.experts)) {
      return {
        data: heuristicSearch(resolvedInput),
        meta: {
          model: meta.model,
          provider: meta.provider,
          tokensUsed: meta.tokensUsed,
    ...
    const validIds = new Set(resolvedInput.experts.map((e) => e.id));
...
var health = catchAsync(async (_req, res) => {
  ...
  const httpStatus6 = probe.status === "ok" ? 200 : probe.status === "unconfigured" ? 503 : 503;
  res.status(httpStatus6).json({
...
var search3 = z13.object({
  body: z13.object({
    ...
    experts: z13.array(expertItem).max(200).optional(),
    industries: z13.array(z13.string().max(120)).max(100).optional()
  })
});
