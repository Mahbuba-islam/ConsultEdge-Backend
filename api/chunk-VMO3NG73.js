...
var changePassword = async (payload, authContext) => {
  const { sessionToken, authorizationHeader, cookieHeader } = authContext;
  if (!sessionToken && !authorizationHeader && !cookieHeader) {
  ...
  if (authorizationHeader) {
    ...
    headerInit.Authorization = `Bearer ${sessionToken}`;
  }
  if (cookieHeader) {
    headerInit.Cookie = cookieHeader;
  } else if (sessionToken) {
    headerInit.Cookie = `better-auth.session_token=${sessionToken}; __Secure-better-auth.session_token=${sessionToken}`;
  }
  const authHeaders = new Headers(headerInit);
