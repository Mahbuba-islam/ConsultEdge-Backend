...
var getAdminStats = async () => {
  ...
  const formattedStatus = consultationStatusDistribution.map(({ status: status33, _count }) => ({
    status: status33,
    count: _count.id
  }));
...
// src/lib/socket.ts
var ioInstance = null;
var chatWsHubInstance = null;
var setSocketIO = (io) => {
  ioInstance = io;
};
var getSocketIO = () => ioInstance;
var setChatWsHub = (hub) => {
  chatWsHubInstance = hub;
};
var getChatWsHub = () => chatWsHubInstance;
...
router18.use("/users", userRouter);
router18.use("/experts", expertRouter);
router18.use("/clients", clientRouter);
router18.use("/schedules", scheduleRoutes);
router18.use("/expert-schedules", expertScheduleRouter);
router18.use("/consultations", consultationRouter);
...
router18.use("/chat", chatRoutes);
router18.use("/conversations", conversationsRoutes);
router18.use("/ai", aiRoutes);
router18.use("/industries", industryRouter);
router18.use("/expert-verification", expertVerificationRouter);
router18.use("/testimonials", testimonialRoutes);
