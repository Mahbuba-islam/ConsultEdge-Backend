...
// src/modules/ai/prompts/insightsPrompt.ts
var insightsPrompt = (input) => `
...
var heuristicRecommendations = (input) => {
  ...
  const scored = input.experts.map((e) => {
    let score = 0.4;
  ...
  const industries = (input.industries ?? []).filter((i) => interests.has(i.toLowerCase())).slice(0, 5).map((name) => ({ name, reason: "Matches your recent activity" }));
  return {
  ...
  try {
    const { data, meta } = await aiProvider.generateJSON({
      messages: [
        {
          role: "system",
          content: "You are ConsultEdge's recommendation engine. Always return strict JSON."
        },
    ...
    data.recommendedExperts = data.recommendedExperts.filter((r) => validIds.has(r.id)).slice(0, 8);
    return {
      data,
      meta: {
        model: meta.model,
...
var recommendations3 = z13.object({
  body: z13.object({
    userBehavior: z13.object({
      recentSearches: z13.array(z13.string().max(200)).max(20).optional(),
      viewedExperts: z13.array(z13.string()).max(50).optional(),
      industries: z13.array(z13.string().max(120)).max(20).optional()
    }).default({}),
    experts: z13.array(expertItem).min(1).max(200),
