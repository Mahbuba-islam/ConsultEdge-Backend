...
var normalizePagination = (query) => {
  const page = Math.max(1, Number(query.page) || 1);
  const limit = Math.min(100, Math.max(1, Number(query.limit) || 10));
  const skip = (page - 1) * limit;
  return { page, limit, skip };
};
var normalizeSort = (query) => {
...
var normalizeFilters = (query) => {
  const statusValue = typeof query.status === "string" ? query.status.toUpperCase() : void 0;
  const status36 = statusValue && Object.values(ExpertApplicationStatus).includes(statusValue) ? statusValue : void 0;
  return {
    status: status36,
    industryId: typeof query.industryId === "string" ? query.industryId : void 0,
    reviewedBy: typeof query.reviewedBy === "string" ? query.reviewedBy : void 0,
    searchTerm: typeof query.searchTerm === "string" ? query.searchTerm.trim() : void 0
  };
};
...
var submitApplication = async (userId, payload) => {
  ...
  if (!payload.resume?.resumeUrl) {
    throw new AppError_default(status4.BAD_REQUEST, "Resume file is required");
  }
  ...
  const createdApplication = await prisma.$transaction(async (tx) => {
    const application = await tx.expertApplication.create({
      data: {
        ...
        resumeUrl: payload.resume.resumeUrl,
        resumeFileName: payload.resume.resumeFileName,
        resumeFileType: payload.resume.resumeFileType,
        resumeFileSize: payload.resume.resumeFileSize
      },
      include: {
        industry: true
      }
...
var applyExpert = async (userId, payload) => {
  ...
  if (!payload.resume?.resumeUrl) {
    throw new AppError_default(status8.BAD_REQUEST, "Resume file is required");
  }
  const industry = await prisma.industry.findUnique({
    where: { id: industryId, isDeleted: false },
    select: { id: true }
...
var applyExpert2 = catchAsync(async (req, res) => {
  ...
  const resume = mapUploadedResume(req.file);
