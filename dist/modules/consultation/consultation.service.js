...
import { bookingSearchableFields, bookingFilterableFields, bookingIncludeConfig, } from "./consultation.constant";
...
const sendConsultationNotifications = async ({ type, clientUserId, expertUserId, clientMessage, expertMessage, }) => {
    const notifications = [
...
const getSessionMeta = (consultation) => {
    ...
    const scheduledEnd = consultation.expertSchedule?.schedule?.endDateTime ??
        new Date(scheduledStart.getTime() + 60 * 60 * 1000);
    const joinAvailableFrom = new Date(scheduledStart.getTime() - SESSION_JOIN_LEAD_MINUTES * 60 * 1000);
    const joinAvailableUntil = new Date(scheduledEnd.getTime() + SESSION_JOIN_GRACE_MINUTES * 60 * 1000);
    const now = new Date();
...
const enrichConsultation = (consultation) => ({
    ...consultation,
    sessionMeta: getSessionMeta(consultation),
});
const getConsultationWithAccess = async (consultationId, user) => {
...
const validateBookableExpertSchedule = async (expertId, expertScheduleId) => {
    ...
    if (!expertSchedule) {
        throw new AppError(status.NOT_FOUND, "The selected availability slot was not found for this expert");
    }
    ...
    if (expertSchedule.isBooked) {
        throw new AppError(status.BAD_REQUEST, "This schedule is already booked for another consultation");
    }
...
const syncUnpaidConsultationsWithStripe = async (where) => {
    ...
    const consultationMap = new Map(unpaidConsultations
...
const bookConsultation = async (payload, user) => {
    ...
    const expertSchedule = await validateBookableExpertSchedule(expert.id, payload.expertScheduleId);
...
const bookConsultationWithPayLater = async (payload, user) => {
    const client = await prisma.client.findUniqueOrThrow({
        where: { userId: user.userId },
    });
    const expert = await prisma.expert.findUniqueOrThrow({
...
const initiateConsultationPayment = async (consultationId, user) => {
    const client = await prisma.client.findUniqueOrThrow({
        where: { userId: user.userId },
    });
    const consultation = await prisma.consultation.findFirst({
    ...
    if (consultation.payment.status === PaymentStatus.PAID) {
        throw new AppError(status.BAD_REQUEST, "Payment already completed for this consultation");
    }
    if (consultation.status === ConsultationStatus.CANCELLED ||
        consultation.status === ConsultationStatus.COMPLETED ||
        consultation.status === ConsultationStatus.ONGOING) {
        throw new AppError(status.BAD_REQUEST, "Payment cannot be initiated for a cancelled, completed, or ongoing consultation.");
    }
    const successParams = new URLSearchParams({
        consultationId: consultation.id,
...
const getSessionAccess = async (consultationId, user) => {
...
const startSession = async (consultationId, user) => {
    const consultation = await getConsultationWithAccess(consultationId, user);
    if (consultation.status === ConsultationStatus.CANCELLED) {
...
const completeSession = async (consultationId, user, payload) => {
    const consultation = await getConsultationWithAccess(consultationId, user);
    if (consultation.status === ConsultationStatus.CANCELLED) {
    ...
    if (consultation.status !== ConsultationStatus.ONGOING &&
        ...
        throw new AppError(status.BAD_REQUEST, "Only paid or ongoing consultations can be completed.");
    }
    const updatedConsultation = await prisma.consultation.update({
        where: { id: consultation.id },
        data: {
...
const cancelConsultation = async (consultationId, user, payload) => {
    const consultation = await getConsultationWithAccess(consultationId, user);
    if (consultation.status === ConsultationStatus.CANCELLED) {
        return enrichConsultation(consultation);
    }
    ...
    if (consultation.status === ConsultationStatus.ONGOING && user.role !== Role.ADMIN) {
        throw new AppError(status.BAD_REQUEST, "An ongoing session cannot be cancelled. Please complete it instead.");
    }
...
const rescheduleConsultation = async (consultationId, user, payload) => {
    const consultation = await getConsultationWithAccess(consultationId, user);
    if (consultation.status === ConsultationStatus.CANCELLED) {
    ...
    if (consultation.expertScheduleId === payload.newExpertScheduleId) {
        throw new AppError(status.BAD_REQUEST, "Please choose a different schedule for rescheduling.");
    }
    ...
    if (newExpertSchedule.schedule.startDateTime <= new Date()) {
        throw new AppError(status.BAD_REQUEST, "Please choose a future schedule slot for rescheduling.");
    }
    const updatedConsultation = await prisma.$transaction(async (tx) => {
        await tx.expertSchedule.update({
            where: { id: consultation.expertScheduleId },
        ...
        return tx.consultation.update({
            ...
            data: {
                ...
                status: consultation.paymentStatus === PaymentStatus.PAID
...
const updateConsultationStatus = async (consultationId, user, payload) => {
    ...
    const consultation = await getConsultationWithAccess(consultationId, user);
    if (consultation.status === ConsultationStatus.CANCELLED) {
        throw new AppError(status.BAD_REQUEST, "Cancelled consultations cannot be updated to another status.");
    }
    if (consultation.status === ConsultationStatus.COMPLETED) {
        throw new AppError(status.BAD_REQUEST, "Completed consultations cannot be updated to another status.");
    }
    ...
    if (nextStatus === ConsultationStatus.CONFIRMED) {
        if (consultation.paymentStatus !== PaymentStatus.PAID) {
            throw new AppError(status.BAD_REQUEST, "Only paid consultations can be confirmed.");
        }
        const updatedConsultation = await prisma.consultation.update({
            where: { id: consultation.id },
            data: {
    ...
    if (nextStatus === ConsultationStatus.PENDING) {
        if (user.role !== Role.ADMIN) {
            throw new AppError(status.FORBIDDEN, "Only admins can set consultation status to pending.");
        }
        const updatedConsultation = await prisma.consultation.update({
            where: { id: consultation.id },
            data: {
...
export const getAllConsultationsAdmin = async (query) => {
