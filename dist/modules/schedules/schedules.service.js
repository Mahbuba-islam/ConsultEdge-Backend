...
const getActiveExpertByUserId = async (userId) => {
    ...
    throw new AppError(status.FORBIDDEN, "Only expert accounts can manage their own schedule catalog");
};
//createSchedules
...
//   return schedules;
// };
const createSchedules = async (payload, user) => {
    const { startDate, endDate, startTime, endTime } = payload;
    const interval = 30;
    const schedules = [];
    ...
    const [eh, em] = endTime.split(":").map(Number);
    while (currentDate <= lastDate) {
        let cursor = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), sh, sm, 0, 0);
        const endDateTime = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), eh, em, 0, 0);
    ...
    if (user?.role === Role.EXPERT && user.userId && schedules.length) {
        const expert = await getActiveExpertByUserId(user.userId);
        await prisma.$transaction(schedules.map((schedule) => prisma.expertSchedule.upsert({
            where: {
                expertId_scheduleId: {
                    expertId: expert.id,
                    scheduleId: schedule.id,
...
//getAllSchedules
const getAllSchedules = async (query, user) => {
    let baseQuery = {
        isDeleted: false,
    };
    ...
    const qb = new QueryBuilder(prisma.schedule, query, {
...
const getPublishedSchedulesByExpertId = async (expertId) => {
...
const updateSchedule = async (id, payload) => {
    ...
    const [sh, sm] = startTime.split(":").map(Number);
    const [eh, em] = endTime.split(":").map(Number);
    const startDateTime = new Date(baseStart.getFullYear(), baseStart.getMonth(), baseStart.getDate(), sh, sm, 0, 0);
    const endDateTime = new Date(baseEnd.getFullYear(), baseEnd.getMonth(), baseEnd.getDate(), eh, em, 0, 0);
...
const deleteSchedule = async (id) => {
    ...
    if (isBooked) {
        throw new AppError(status.BAD_REQUEST, "Cannot delete schedule — it is already booked");
    }
