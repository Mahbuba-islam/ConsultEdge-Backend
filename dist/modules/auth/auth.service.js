...
const isBetterAuthLikeError = (error) => {
...
const mapBetterAuthError = (error, fallbackMessage) => {
...
const loginUser = async (payload) => {
    ...
    const data = await auth.api
        ...
        .catch((error) => {
        const mappedError = mapBetterAuthError(error, "Invalid email or password");
        if (mappedError) {
            throw mappedError;
        }
        throw error;
...
// }
// } 
const changePassword = async (payload, authContext) => {
    ...
    if (currentPassword && currentPassword === newPassword) {
        throw new AppError(status.BAD_REQUEST, "New password must be different from current password");
    }
