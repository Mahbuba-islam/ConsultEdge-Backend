...
const getActiveExpertByUserId = async (userId) => {
    ...
    if (user?.role === Role.EXPERT) {
        throw new AppError(httpStatus.BAD_REQUEST, "Your account role is EXPERT, but your expert profile is missing or inactive. Please complete expert profile setup first.");
    }
    throw new AppError(httpStatus.FORBIDDEN, "Only expert accounts can manage availability schedules");
};
// ===============================
// ASSIGN SCHEDULES TO EXPERT
// ===============================
const assignExpertSchedules = async (userId, payload) => {
    ...
    for (const scheduleId of payload.scheduleIds) {
        ...
        if (!schedule)
            throw new AppError(httpStatus.NOT_FOUND, `Schedule not found: ${scheduleId}`);
...
// ===============================
const getMyExpertSchedules = async (userId, query) => {
    const expert = await getActiveExpertByUserId(userId);
    const qb = new QueryBuilder(prisma.expertSchedule, { expertId: expert.id, isDeleted: false, ...query }, {
...
// ===============================
const updateMyExpertSchedules = async (userId, payload) => {
    const expert = await getActiveExpertByUserId(userId);
...
// ===============================
const publishMyExpertSchedules = async (userId, payload) => {
    const expert = await getActiveExpertByUserId(userId);
    ...
    if (schedules.length !== payload.scheduleIds.length) {
        throw new AppError(httpStatus.NOT_FOUND, "One or more schedule mappings were not found for this expert");
    }
...
// ===============================
const getPublishedExpertSchedules = async (expertId) => {
...
// ===============================
const deleteMyExpertSchedule = async (userId, scheduleId) => {
    const expert = await getActiveExpertByUserId(userId);
    ...
    if (existing.isBooked)
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot delete a booked schedule");
...
//# sourceMappingURL=expertSchdules.service.js.map