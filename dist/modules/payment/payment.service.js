...
const findPaidCheckoutSessionByMetadata = async (payload) => {
...
const confirmConsultationPaymentSuccess = async (payload, user) => {
    ...
    if (consultation.payment.id !== payload.paymentId ||
    ...
    if (payload.sessionId) {
        ...
        const metadataMatches = session.metadata?.consultationId === payload.consultationId &&
            session.metadata?.paymentId === payload.paymentId &&
            session.metadata?.transactionId === payload.transactionId;
    ...
    if (!paidSession) {
        throw new AppError(status.BAD_REQUEST, "Stripe has not confirmed this payment yet. Please retry shortly.");
    }
    const updated = await prisma.$transaction(async (tx) => {
        const updatedConsultation = await tx.consultation.update({
            ...
            data: {
                ...
                status: consultation.status !== ConsultationStatus.CANCELLED &&
...
const handleStripeWebhookEvent = async (event) => {
    ...
    switch (event.type) {
        case "checkout.session.completed": {
            ...
            console.log(`Payment ${session.payment_status} for consultation ${consultationId}`);
            return result;
        }
